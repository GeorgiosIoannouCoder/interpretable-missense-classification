...
\begin{tabular}{@{}p{1.95cm}p{1.2cm}p{4.05cm}p{4.15cm}@{}}
\toprule
...
ESM-2 head & 1280 & MLP $d\to256\to1$, LayerNorm, dropout 0.2, GELU & AdamW + cosine (Appendix~\ref{app:training_details}) \\
...
\caption{\textbf{Test-set ROC and PR curves (Figure 2).} All models on $n=38{,}526$ gene-held-out test variants (AlphaMissense on 37{,}470 with score coverage). The ESM-2 head sits between the handcrafted baselines and AlphaMissense.}
\label{fig:rocpr}
\end{figure}
...
\centering
\small
\caption{\textbf{Table A2 / Calibration on the gene-held-out test set ($n=38{,}526$).} Brier score and expected calibration error (ECE) with ten quantile bins; lower is better. Plots appear in Figure~\ref{fig:calibration}.}
...
\caption{\textbf{Figure A2 / Reliability diagrams.} Calibration curves for the four in-house scorers (10 quantile bins). Points should hug the diagonal under perfect calibration.}
...
\caption{\textbf{Figure A3 / Random forest feature-family ablation.} Test AUPRC when training only on cumulative feature subsets (top block) or removing one family from the full 62-d vector (bottom block) using the Phase~7 hyperparameters. The local window composition drives most of the lift beyond substitution-aware features.}
\label{fig:ablation}
\end{figure}
...
\caption{\textbf{Figure A4 / Random forest vs.\ ESM-2 head disagreements.} \emph{Left:} test-set scores with validation-tuned thresholds (dashed); black outlines mark variants where hard predictions differ. \emph{Right:} among those disagreements ($n=9{,}552$), how often each model matches the ClinVar binary label (6{,}801 vs.\ 2{,}751).}
